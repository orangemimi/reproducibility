  ...
  <div class="main">
    ...
    <div class="row-style">
      ...
      >
        ...
        <el-table-column label="selection" width="250" v-if="role == 'builder'">
          <template #default="scope">
            <el-checkbox v-model="scope.row.checked" :label="scope.row.name" :key="scope.row.id" @change="handleCheckAllChange(scope.row)"></el-checkbox>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    async getSelectedData() {
      ...
      this.dataItemListDirect.forEach(item => {
        this.setCheckedData(item, dataSelected);
      });
      this.$forceUpdate();
      // this.toggleSelection(this.multipleSelection);
    },

    setCheckedData(item, dataSelected) {
      if (dataSelected.some(selection => selection.id == item.id)) {
        item.checked = true;
      } else {
        item.checked = false;
      }
      if (item.folder && item.children != null && item.children.length != 0) {
        item.children.forEach(child => this.setCheckedData(child, dataSelected));
      }
    },
    ...
    handleCheckAllChange(scope) {
      ...
      if (scope.children != null && scope.children.length != 0) {
        this.handleCheckAll(scope, scope.checked);
      }
      // 查找父级选框
      // this.getParent(this.tableData, scope.id).forEach(item => {
      //   // 判断父级是否有子元素
      //   console.log(item);
      //   if (!item.children) {
    ...
    handleCheckAll(row, checked) {
      ...
      if (row.children != null && row.children.length != 0) {
        row.children.forEach(child => {
          ...
          this.handleCheckAll(child, checked);
          this.$forceUpdate();
        });
      }
      // this.$forceUpdate();
    ...
    async submitBtn() {
      ...
      //用id重组一个新数组
      let filter = [];
      this.dataItemListDirect.forEach(ele => {
        if (ele.checked) {
          filter.push(ele.id);
        }
      });
