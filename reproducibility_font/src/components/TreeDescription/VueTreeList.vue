<template>
  <div class="vtl">
    <div v-if="model.name !== 'root'" :id="model.id" class="vtl-node" :class="{ 'vtl-leaf-node': model.isLeaf, 'vtl-tree-node': !model.isLeaf }">
      ...
      >
        ...
        <div v-else class="resource">
          <!-- <input class="vtl-input" type="text" ref="nodeInput" :value="model.name" @input="updateName" @blur="setUnEditable" /> -->
          <div class="name">
            <el-input v-model="model.name" size="mini" />
          </div>
          <div class="value">
            <el-input v-model="model.value" size="mini" />
          </div>
        </div>
        <div class="vtl-operation" v-show="isHover">
          <span :title="defaultAddTreeNodeTitle" @click.stop.prevent="getChildForm" v-if="!model.isLeaf && !model.addTreeNodeDisabled">
          ...
          <span title="edit" @click.stop.prevent="editable = true" v-if="!model.editNodeDisabled">
    ...
    <div v-if="isFolder" :class="{ 'vtl-tree-margin': model.name !== 'root' }" v-show="model.name === 'root' || expanded">
      <item
    ...
    <el-dialog :visible.sync="editModelDialogShow" width="50%" title="Configuration">
      <el-form ref="form" :model="editModelForm" label-width="80px">
        <el-form-item label="Key">
          <el-input v-model="editModelForm.name"></el-input>
        </el-form-item>
        <el-form-item label="Value">
          <el-input v-model="editModelForm.value"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    editModelSubmit() {
      this.model.name = this.editModelForm.name;
      this.model.value = this.editModelForm.value;
    },

    getChildForm() {
      let id = this.defaultTreeNodeName;
      let isLeaf = false;
      ...
      this.expanded = true;
      var node = new TreeNode({ id, isLeaf, name, value });
